...
import com.learning.java_grpc.bankservicerpc.clientloadbalancer.BankNameResolverProvider;
import com.learning.java_grpc.bankservicerpc.clientloadbalancer.ServiceRegistry;
...
public class BankServiceClientLoadBalancerTest {
	...
	public void setup() {
		//lets register first
		List<String> addresses = Arrays.asList("localhost:6565", "localhost:7575");
		String bankServiceName = "bank-service";
		ServiceRegistry.register(bankServiceName, addresses);
		
		  NameResolverRegistry.getDefaultRegistry().register(new BankNameResolverProvider());
		
		//from now we should use nameresolver and not any specfic address
		
		ManagedChannel managedChannel = ManagedChannelBuilder
				//.forAddress("localhost", 8585)
				.forTarget(bankServiceName)
				.defaultLoadBalancingPolicy("round_robin") // mantodaty other wise it routes all the different grcps to same server instance
				.usePlaintext().build();
